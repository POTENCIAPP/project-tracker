/**
 * AdminUsersPage.tsx
 * Gestión global de accesos: crear (usuario + contraseña + proyecto),
 * listar y eliminar. Reutiliza el shell del panel.
 */
...
export function AdminUsersPage() {
  ...
  return (
    <AdminShell section="usuarios" onTokenSaved={load}>
      {needToken && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
          Pegá el <strong>Token de admin</strong> en el campo de arriba para
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-slate-900">Nuevo acceso</h2>
        </div>
        ...
        {created && (
          <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">
            Acceso creado. Pasale estos datos al cliente (no se vuelven a mostrar):
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        {users === null ? (
          ...
          <p className="px-5 py-12 text-center text-sm text-slate-500">
            Todavía no hay accesos. Creá el primero arriba.
          </p>
        ) : (
